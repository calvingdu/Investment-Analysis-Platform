...
from great_expectations.compatibility.sqlalchemy import functions
...
from great_expectations.expectations.metrics import column_condition_partial
from great_expectations.expectations.metrics import ColumnMapMetricProvider
from great_expectations.render import RenderedStringTemplateContent
from great_expectations.render.renderer.renderer import renderer
from great_expectations.render.util import num_to_str
from great_expectations.render.util import parse_row_condition_string_pandas_engine
...
# This function is used to validate a value against special characters
def not_contain_special_character(val, allowed_characters):
    special_characters = [
...
class ColumnValuesToNotContainSpecialCharacters(ColumnMapMetricProvider):
    ...
    def _pandas(cls, column, allowed_characters: list or set, **kwargs):
        return column.apply(not_contain_special_character, args=(allowed_characters))
    ...
    def _spark(cls, column, allowed_characters: list or set, **kwargs):
        ...
        result_column = F.when(
            not_contain_special_character_udf(column, allowed_characters),
            True,
        ).otherwise(False)
        return result_column

    @column_condition_partial(engine=SqlAlchemyExecutionEngine)
    def _sqlalchemy(cls, column, allowed_characters: list or set, **kwargs):
        # Register the function
        functions.func.not_contain_special_character = not_contain_special_character

        # Apply the function to the column
        result_column = functions.func.cast(
            functions.func.not_contain_special_character(column, allowed_characters),
            Boolean,
        )
        return result_column
