...
class DictToPostgresOperator(BaseOperator):
    ...
    def execute(self, table: str, data_dict: dict,):
        ...
        stmt = stmt.on_conflict_do_nothing(index_elements=[table.primary_key.columns.keys()[0]])

        with engine.connect() as conn:
            conn.execute(stmt)

        self.log.info("Successfully inserted JSON data into the PostgreSQL table: %s", table)

class PandasToPostgresOperator(BaseOperator):
    def __init__(self, table:str, df: pd.DataFrame, postgres_conn_id='POSTGRES_CONN_ID', *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.postgres_conn_id = postgres_conn_id
        self.table = table
        self.df = df
    ...
    def execute(self, no_duplicates=True, **context):
        ...
        if no_duplicates:
            cursor.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = '{self.table}'")
            ...
            with engine.connect() as conn:
                conn.execute(f"CREATE TEMP TABLE {temp_table} AS TABLE {self.table} WITH NO DATA;")
