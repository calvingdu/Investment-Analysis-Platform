...
def get_news(query: str, endpoint: str, from_date: str, to_date: str, sort_by: str, page_size: int, page_number: int, **kwargs) -> dict: 
...
class NewsAPIToDataframeOperator(BaseOperator):
    ...
    def execute(self, **context) -> pd.DataFrame:
        news_result = get_news(
                self.news_topic, self.endpoint, self.from_date, self.to_date,
                self.sort_by, self.page_size, self.page_number
        )
